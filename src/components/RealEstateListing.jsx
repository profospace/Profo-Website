...
const RealEstateListing = ({ details, handleButtonClick }) => {
    const [isLiked, setIsLiked] = useState(false);
        ...
        <div className="max-w-xl mx-auto bg-white rounded-lg shadow-sm py-4 px-6">
            <div className="flex justify-between items-start mb-2">
                <span className="text-gray-500 text-sm">
                    {getTimeSince(details?.createdAt)}, {details?.statistics.totalProperties} properties
                </span>
                <button
                    onClick={() => setIsLiked(!isLiked)}
                    className="text-gray-400 hover:text-red-500"
            ...
            <div className="flex items-center gap-2 mb-4">
                <span className="text-2xl font-bold">{details?.statistics.completedProjects} Projects</span>
                <span className="text-gray-400">{details?.experience} years experience</span>
            </div>
